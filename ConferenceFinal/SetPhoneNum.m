...
@interface SetPhoneNum ()

@end

@implementation SetPhoneNum

@synthesize hostNumEdit;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        CustomLabel *topLabel = [[CustomLabel alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 49) newStyle:@"设置号码"];
        [topLabel.returnBtn addTarget:self action:@selector(returnPrevious) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:topLabel];
        
        [self.hostNumEdit setPlaceholder:@"设置主持人号码"];
    }
    return self;
}
